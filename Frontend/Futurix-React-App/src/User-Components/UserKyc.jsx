...
const UserKyc = () => {
    ...
    const [borderColor, setBorderColor] = useState("#050c1b");
    ...
    return (
        <div className="flex justify-center items-center min-h-screen text-gray-200" style={{ backgroundImage: 'linear-gradient(180deg, #050c1b, #2a4365)', padding: "25px" }}>
            <form onSubmit={handleSubmit(onSubmit)} className="max-w-md mx-auto rounded-3xl shadow-md" style={{ backgroundColor: "#050c1b", padding: "25px"}}>
                <h1 className="text-3xl text-white font-bold text-center mb-4 hover:text-gray-400 transition-colors duration-300 cursor-pointer">KYC Form</h1>
                <div className="grid grid-cols-1 gap-4">
                    <div className="relative">
                        <input
                            ...
                            id="aadhar"
                            name="aadhar"
                            className={`border rounded-md px-3 py-2 w-full shadow-sm shadow-white focus:border-blue-500 focus:ring-blue-500 ${focusedInput === "aadhar" ? 'animate-pulse' : ''} ${errors.aadhar ? 'border-white-500' : ''}`} // Add error class for visual feedback
                            {...register("aadhar", {
                                required: { value: true, message: 'Please Enter your Aadhar card number' },
                                pattern: {
                                    value: /^\d{12}$/,
                                    message: "Aadhar card number must be exactly 12 digits"
                                },
                                validate: value => (value === '' || /^\d+$/.test(value)) || 'Please enter digits only' // Custom validation with immediate feedback
                            })}
                            maxLength={12}
                        ...
                        />
                        {errors.aadhar && <p className="text-amber-400 text-xs mt-1">{errors.aadhar.message}</p>}
                    </div>

                    <div className="flex aadhar-file-chooser"> {/* New class name */}
                    <label htmlFor="aadharFile" className="text-base text-gray-400 py-2 px-0">Upload Your  Aadhar Card </label>
                        <input
                            type="file"
                            id="aadharFile"
                            name="aadharFile"
                            className={`text-white border rounded-md px-2 py-3 w-15 shadow-sm shadow-white focus:border-blue-500 focus:ring-blue-500 ${focusedInput === "aadharFile" ? 'animate-pulse' : ''}`}
                    ...
                    {errors.aadharFile && <p className="text-amber-400 text-xs">{errors.aadharFile.message}</p>}
