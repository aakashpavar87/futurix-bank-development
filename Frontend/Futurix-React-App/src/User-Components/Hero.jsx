...
import NotVerifiedText from "./NotVerified";
...
export const Hero = () => {
  ...
  useEffect(() => {
    ...
    console.log(myUser);
    if (state && error) showToastMessage(state, true);
    else if (state) showToastMessage(state, false);

    if (!myUser?.userData?.account) {
      showToastMessage("Please open your account", true);
    }
    (async () => {
      try {
  ...
  return (
    <div className="flex flex-col">
      <div className="container mx-auto mt-4">
        <h1 className="text-2xl font-bold mb-4">User Verification Status</h1>
        <NotVerifiedText verified={myUser?.userData?.active} />{" "}
        {/* Change verified to false to see the not verified status */}
      </div>
      <section
        id="home"
        ...
        <div className={`flex flex-col mx-3 md:my-0 my-10 relative`}>
          ...
          >
            ...
            {myUser?.userData?.address && (
              <>
                <b>Address :</b> {myUser?.userData?.address?.street}
                <br />
                <b>City :</b>
                {myUser?.userData?.address?.city} <br />
                <b>State :</b> {myUser?.userData?.address?.state} <br />
              </>
            )}
            {myUser?.userData?.account && (
              <>
                <b>Account Number </b> :{" "}
                {myUser?.userData?.account?.accountnumber || "N/A"} <br />
