  ...
  return (
    <div className="flex justify-center items-center min-h-screen text-black" style={{ backgroundImage: 'linear-gradient(180deg, #050c1b, #2a4365)', padding: "25px", borderRadius: "20px" }}>
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-md mx-auto rounded-3xl shadow-md" style={{ backgroundColor: "#050c1b", padding: "25px", borderRadius: "20px" }}>
        ...
        <h1 className="text-3xl text-white font-bold text-center mb-4 hover:text-gray-400 transition-colors duration-300 cursor-pointer">DepositForm</h1>
        ...
        <div className="grid grid-cols-1 gap-4">
          <div className="relative">
            <input
              ...
              id="accountnumber"
              name="accountnumber"
              className={`border rounded-md px-3 py-2 w-full text-black shadow-sm shadow-white focus:border-blue-500 focus:ring-blue-500 ${focusedInput === "accountnumber" ? 'animate-pulse' : ''}`}
              ...
              onBlur={handleBlur}
            />
            {errors.accountnumber && <p className="text-amber-400 text-xs mt-1">Please Enter Correct Account Number</p>}
            <label
        ...
        <div className="relative">
          <input
            ...
            className={`border rounded-md px-3 py-2 w-full shadow-sm shadow-white focus:border-blue-500 focus:ring-blue-500 ${focusedInput === "amount" ? 'animate-pulse' : ''}`}
            style={{ transition: 'background-color 0.3s', background: borderColor }}
            onFocus={() => handleFocus("amount")}
            onBlur={handleBlur}
          />
          {errors.amount && <p className="text-amber-400 text-xs mt-1">Please Enter Your Amount</p>}
        </div>
        ...
        <div className="relative">
          <input
            ...
            id="pin"
            name="pin"
            className={`border rounded-md px-3 py-2 w-full shadow-sm shadow-white focus:border-blue-500 focus:ring-blue-500 ${focusedInput === "pin" ? 'animate-pulse' : ''}`}
            {...register("pin", {
        ...
        {errors.pin && <p className="text-amber-400 text-xs mt-1">{errors.pin.message}</p>}
        <br></br>
        <div className="form-group">
          <label>
            <input type="checkbox" {...register("confirmation", { required: true })} />
            <b className='text-red-600'> I confirm this deposit request</b>
          </label>
          {errors.confirmation && <span>This field is required</span>}
        </div>
        <br></br>
        <div className="flex justify-center">
          <button type="submit" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-4">Deposit</button>
        </div>
      </form>
    </div>
  );
