                ...
                <div className="grid grid-cols-1 gap-4">
                    <div className="relative">
                        <input
                            ...
                            onBlur={handleBlur}
                            onInput={e => {
                                const newValue = e.target.value.replace(/[^0-9]/g, ''); // Remove non-digits
                                if (newValue.length > 12) {
                                    newValue = newValue.substring(0, 12); // Limit to 12 characters
                                }
                                e.target.value = newValue;
                            }} // Dynamic input restriction and error handling
                        />
                        {errors.aadhar && <p className="text-amber-400 text-xs mt-1">{errors.aadhar.message}</p>}
                    </div>


                    <div className="relative aadhar-file-chooser"> {/* New class name */}
                        <input
                            type="file"
                            id="aadharFile"
                            name="aadharFile"
                            className={`border rounded-md px-3 py-2 w-full shadow-sm shadow-white focus:border-blue-500 focus:ring-blue-500 ${focusedInput === "aadharFile" ? 'animate-pulse' : ''}`}
